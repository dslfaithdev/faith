...
<tr>
	...
	<tr>
		...
		<tr>
			...
			Social Network Transformation - Pending Friend Requests</td><td colspan="2">
			</td>
			<td width="5%"></td>
		</tr>
		<tr>
			...
			Accept your friends' friend requests to help FAITH to transform the real Facebook social graph to virtual FAITH social graph and outputs the transformed graph to the applications behide FAITH.
			</td>
			<td width="5%"></td>
		</tr>
<?php
...
try
{
	...
	try
	{
		...
		while($row = mysql_fetch_array($results))
		{
			...
			get_pending_friendship_contents($transform_add_id,
									    	$add_uid_a,
									    	$add_time,
									    	$social_path);
			$div_counter++;
		...
		if($div_counter == 0)
		{
			echo '<tr><td><br /><br /><h5>You have no friend requests.</h5></td></tr>';
		}
	}
	catch (Exception $e)
	{
...
<?php 
	...
	function get_pending_friendship_contents($transform_add_id,
									     	 $add_uid_b,
									     	 $add_time,
									     	 $social_path)
	{
		...
		<tr>
			...
			<tr>
				...
				<form style="padding-left: 20px; padding-right: 5px;display: inline;" name="confirm_request_form'.$transform_add_id.'" id="confirm_request_form'.$transform_add_id.'"
					action="set_policy_transform_pending_request.php" method="post">
	    		<input type="hidden" name="add_id" value="'.$transform_add_id.'"/>
	    		<input type="hidden" name="status" VALUE="Confirm">
	    		<font style="padding-left: 5px;padding-right: 5px;">
	    		<a class="AcceptButtonStyle"
			 	href="#" onclick="document.getElementById('."'confirm_request_form".$transform_add_id."'".').submit();">Confirm</a>
	    		</font>
	       		</form>
