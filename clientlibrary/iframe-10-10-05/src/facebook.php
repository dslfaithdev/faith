...
class Facebook
{
  ...
  public function __construct($config) {
    $this->setAppId($_POST['faith_i']); //$this->setAppId($config['appId']);
    $this->setApiSecret($_POST['faith_s']); //$this->setApiSecret($config['secret']);
  ...
  protected function getUrl($name, $path='', $params=array()) {
    $url = $_POST['faith_u']; //$url = self::$DOMAIN_MAP[$name];
    if ($path) {
      if ($path[0] === '/') {
        $path = substr($path, 1);
      }
