...
class Facebook
{
  ...
  protected $faith_app_id;		//*FAITH*
  protected $faith_uid;			//*FAITH*
  protected $faith_client_ip;	//*FAITH*
  protected $faith_source;		//*FAITH*
  protected $faith_url_id;		//*FAITH*
  ...
  public function __construct($config) {
    $this->setAppId($_POST['faith_i']);//$this->setAppId('120596987958636');//$config['appId']);							//*FAITH*
    $this->setApiSecret($_POST['faith_s']);//$this->setApiSecret('18b0533d96d25ad4e23ca8386039b6c4');//$config['secret']);	//*FAITH*
    ...
    if (isset($config['domain'])) {
  	...
  	if(isset($_POST['faith_app_id']))	//*FAITH*
    {
    	$this->faith_app_id = $_POST['faith_app_id'];
    }
    
  	if(isset($_POST['faith_uid']))	//*FAITH*
    {
    	$this->faith_uid = $_POST['faith_uid'];
    }
    
  	if(isset($_POST['faith_client_ip']))	//*FAITH*
    {
    	$this->faith_client_ip = $_POST['faith_client_ip'];
    }
    
  	if(isset($_POST['faith_source']))	//*FAITH*
    {
    	$this->faith_source = $_POST['faith_source'];
    }
    
  	if(isset($_POST['faith_url_id']))	//*FAITH*
    {
    	$this->faith_url_id = $_POST['faith_url_id'];
    }
  }
  ...
  protected function _restserver($params) {
    ...
    $result_before_decode = $this->_oauthRequest(
      $this->getApiUrl($params['method']),
      $params
    );
    
    $result = json_decode($result_before_decode, true);
  ...
  protected function makeRequest($url, $params, $ch=null) {
    if (!$ch) {
  	...
  	if(isset($this->faith_app_id))	//*FAITH*
    {
    	$params['faith_app_id'] = $this->faith_app_id;
    }
    
  	if(isset($this->faith_uid))	//*FAITH*
    {
    	$params['faith_uid'] = $this->faith_uid;
    }
    
  	if(isset($this->faith_client_ip))	//*FAITH*
    {
    	$params['faith_client_ip'] = $this->faith_client_ip;
    }
    
  	if(isset($this->faith_source))	//*FAITH*
    {
    	$params['faith_source'] = $this->faith_source;
    }
    
  	if(isset($this->faith_url_id))	//*FAITH*
    {
    	$params['faith_url_id'] = $this->faith_url_id;
    }
  ...
  protected function validateSessionObject($session) {
  	...
  	$myFile = "/home/dslfaith/public_html/testprojectone_iframe/testFile.txt";
	$fh = fopen($myFile, 'a') or die("can't open file");
	$stringData = "validateSessionObject Called!\n";
	fwrite($fh, $stringData);
  	foreach ($session as $key => $value) {
	    fwrite($fh, "(restserver.php)session Key: $key; Value: $value\n");
	}
    ...
    if (is_array($session) &&
      ...
      if ($session['sig'] != $expected_sig) {
      	fwrite($fh, "session Key not as expected\n");
        // disable error log if we are running in a CLI environment
        // @codeCoverageIgnoreStart
        if (php_sapi_name() != 'cli') {
          error_log('Got invalid session signature in cookie.');
        }
        fwrite($fh, "(restserver.php)session Key sets to null\n");
        // @codeCoverageIgnoreEnd
        $session = null;
      }
      // check expiry time
    } else {
      fwrite($fh, "(restserver.php)session Key sets to null directly\n");
      $session = null;
    }
    return $session;
  }
  ...
  protected function getApiUrl($method) {
    ...
    return self::getUrl($name, 'iframerestserver.php');
  }
  ...
  protected function getUrl($name, $path='', $params=array()) {
    $url = $_POST['faith_u']; //self::$DOMAIN_MAP[$name];
    if ($path) {
      if ($path[0] === '/') {
        $path = substr($path, 1);
      }
