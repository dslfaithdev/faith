...
<tr>
	...
	<tr>
		...
		<tr>
			...
			at RESTful API Level - Remove Friends from This Setting</td><td colspan="2">
			</td>
			<td width="5%"></td>
		</tr>
		<tr>
...
try
{
	if(isset($_GET['api_id']) && strlen($_GET['api_id']) > 0)
	{
		...
		if($_POST['select_application_submit'] == 'select')
		{
			...
			$api_results = mysql_query("SELECT Count(not_apply_app_uid) as count_num FROM user_disable_api_app
																	        		WHERE user_disable_api_app.uid = $user_id AND
																	        			  user_disable_api_app.restapi_id = $api_id AND
																	        		   	  user_disable_api_app.not_apply_app_uid = $not_apply_app_uid;", $db);
			...
			if($api_num == '0')
			{
				...
				$time_added = date("Y-m-d H:i:s");
 				...
 				$query = sprintf("INSERT INTO user_disable_api_app (uid, 
													 			    restapi_id,
													 			    not_apply_app_uid,
													 			    time_added) 
														 		    VALUES('%s', '%s', '%s', '%s')",
		...
		else if($_POST['remove_from_list_submit'] == 'remove')
		{
			...
			$api_app_results = mysql_query("DELETE FROM user_disable_api_app
						  						   WHERE user_disable_api_app.uid = $user_id AND
						  						   		 user_disable_api_app.restapi_id = $api_id AND
            			  								 user_disable_api_app.not_apply_app_uid = $not_apply_app_uid;", $db);
 			if(!$api_app_results)
		...
		while($api_row = mysql_fetch_array($api_results))
		{
			...
			<tr>
				<td>
				Hi <fb:name uid="'.$user_id.'" useyou="false" linked="true" />, please select the applications
				which you do NOT want the following rule apply to
				</td>
			</tr>
			<tr>
				<td>
...
?>
	...
	function get_paging_contents($pageNum, $maxPage)
	{
		...
		for($i = $start_paging_num; $i <= $end_paging_num; $i++)
		{
			...
			else
			{
				...
				$paging_html .= 
				'
				<form style="display: inline" action="set_policy_not_app.php?api_id='.$api_id.'&page='.$i.'" name="select_app_paging_form'.$i.'" id="select_app_paging_form'.$i.'" method="post">
	...
	function get_live_search_contents($user_id)
	{
		...
		<input type="text" name="search_txt" id="search_txt" maxlength="30" style="width: 450px;"
			onkeyup="do_add_app_ajax('."'live_search_Div'".',66, '.$user_id.','.$api_id.');" value="'.$search_text.'"></input>
		</td></tr>
		<tr><td>
		...
		<tr><td height="50px" style="vertical-align: top;background-color: #eceff6;">
			...
			<tr>
				...
				<input type="radio" name="app_select_input" id="app_select_input2" value="3" '.$app_select3.'>Facebook Connect Only</input><br />
				<input type="radio" name="app_select_input" id="app_select_input3" value="4" '.$app_select4.'>ALL</input>
				</td>
			</tr>
			</table>
		</td></tr>
...
try
{
	if(isset($_GET['api_id']) && strlen($_GET['api_id']) > 0)
	{
		...
		try
		{
			...
			$results = mysql_query("SELECT user_disable_api_app.uid,
										   user_disable_api_app.restapi_id,
										   user_disable_api_app.not_apply_app_uid,
			...
			while($row = mysql_fetch_array($results))
			{
				...
				get_NOT_apply_application_contents($api_id,
											  	   $not_apply_app_uid,
											       $time_added,
											       $default_page,
			  							   		   $app_name,
...
<?php 
	...
	function get_NOT_apply_application_contents($api_id,
										        $not_apply_app_uid,
										        $time_added,
										        $default_page,
			  							   		$app_name,
...
<!--
function do_add_app_ajax(div,val,uid,api_id) {
	...
	var params={"action":'select',"option":val,"searchwords":document.getElementById('search_txt').getValue(),"otherval":uid,"app_select":select_app_value,"api_id":api_id};
