...
<tr>
	...
	<tr>
		...
		<tr>
			...
			Social Network Transformation - Add Friendship Connections</td><td colspan="2">
			</td>
			<td width="5%"></td>
		</tr>
		<tr>
...
try
{
	if(isset($_GET['id']) && strlen($_GET['id']) > 0)
	{
		...
		if($_POST['select_application_submit'] == 'select')
		{
			...
			$app_results = mysql_query("SELECT Count(not_apply_app_uid) as count_num FROM transform_add_app
																	        		WHERE transform_add_app.transform_add_id = $add_id AND
																	        		   	  transform_add_app.not_apply_app_uid = $not_apply_app_uid;", $db);
			...
			if($app_num == '0')
			{
				...
				$time_added = date("Y-m-d H:i:s");
 				...
 				$query = sprintf("INSERT INTO transform_add_app (transform_add_id, 
													 			 not_apply_app_uid,
													 			 time_added) 
														 		 VALUES('%s', '%s', '%s')",
		...
		else if($_POST['remove_from_list_submit'] == 'remove')
		{
			...
			$add_app_results = mysql_query("DELETE FROM transform_add_app
						  						   WHERE transform_add_app.transform_add_id = $add_id AND
            			  								 transform_add_app.not_apply_app_uid = $not_apply_app_uid;", $db);
 			if(!$add_app_results)
		    {
		...
		while($add_row = mysql_fetch_array($add_results))
		{
			...
			<tr>
				<td colspan="3">
				Hi <fb:name uid="'.$add_uid_a.'" useyou="false" linked="true" />, please select the applications
...
?>
	...
	function get_paging_contents($pageNum, $maxPage)
	{
		...
		for($i = $start_paging_num; $i <= $end_paging_num; $i++)
		{
			...
			else
			{
				...
				$paging_html .= 
				'
				<form style="display: inline" action="set_policy_transform_add_not_app.php?id='.$add_id.'&page='.$i.'" name="select_app_paging_form'.$i.'" id="select_app_paging_form'.$i.'" method="post">
	...
	function get_live_search_contents($user_id)
	{
		...
		<input type="text" name="search_txt" id="search_txt" maxlength="30" style="width: 450px;"
			onkeyup="do_add_app_ajax('."'live_search_Div'".',44, '.$user_id.','.$add_id.');" value="'.$search_text.'"></input>
		</td></tr>
		<tr><td>
...
try
{
	if(isset($_GET['id']) && strlen($_GET['id']) > 0)
	{
		...
		try
		{
			...
			$results = mysql_query("SELECT transform_add_app.not_apply_app_uid,
			...
			while($row = mysql_fetch_array($results))
			{
				...
				get_NOT_apply_application_contents($add_id,
											  	   $not_apply_app_uid,
											       $time_added,
											       $default_page,
			  							   		   $app_name,
...
<?php 
	...
	function get_NOT_apply_application_contents($add_id,
										        $not_apply_app_uid,
										        $time_added,
										        $default_page,
			  							   		$app_name,
...
<!--
function do_add_app_ajax(div,val,uid,add_id) {
	...
	var params={"action":'select',"option":val,"searchwords":document.getElementById('search_txt').getValue(),"otherval":uid,"app_select":select_app_value,"add_id":add_id};
