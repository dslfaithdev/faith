...
try
{
	...
	$homepage = file_get_contents('http://169.237.6.102/~dslfaith/testapplicationone/restapitest/notifications_sendemail.php/', false, $context);
	echo $homepage;
	...
	try
	{
		$result = $facebook->api_client->notifications_sendEmail($user_id,
																 'test email from FAITH',
																 'test email content from FAITH');
		echo "<br>facebook->api_client->notifications_sendEmail(user_id,
																 'test email from FAITH',
																 'test email content from FAITH') REST API CALLED <br>";
